...
def main():
    ...
    for k in range(88):
        oct = (k+9) // 12
        note = f"{keys_s[k%12]}{oct}"
        piano_s.append(note)
        note = f"{keys_f[k%12]}{oct}"
        piano_f.append(note)
    
    fn = sys.argv[1]
    if fn[:5] == 'http:':
        f = urllib3.urlopen(fn)
    else:
        f = open(fn)
    ...
    for l in f:
        ...
        if 'K:' in l and sel:
            ...
            for fs in fsrange:
                for oct in range(9):
                    global_sharps_flats[f'{flats_and_sharps[fs]}{oct}'] = sign
    ...
    if not sel:
        ...
        print(f"*** Song {num} not found in file {fn}!")
